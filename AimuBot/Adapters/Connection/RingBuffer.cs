    ...
    ICollection
{
    ...
    public void AddMany(T[] toAdd, int count = 0)
    {
        if (count == 0) count = toAdd.Length;
        if (count + size > Capacity) throw new System.InvalidOperationException("Buffer is full.");
    ...
    {
        if (count > size)
            throw new System.InvalidOperationException("Cannot peek more than size");

        var r = new T[count];
        int index = head;
        for (int i = 0; i < count; i++, index = (index + 1) % Capacity)
        {
            r[i] = buffer[index];
        }
    ...
    {
        if (count > size)
            throw new System.InvalidOperationException("Cannot take more than size");
        var r = new T[count];
        int index = head;
        for (int i = 0; i < count; i++, index = (index + 1) % Capacity)
        {
