...
internal class Analyzer
{
    ...
    public void AnalyzeNotes()
    {
        ...
        arcs.Add(0, new List<ArcaeaAffArc>());
        arcs.Add(1, new List<ArcaeaAffArc>());
        arcs.Add(2, new List<ArcaeaAffArc>());
        ...
        foreach (var ev in _affReader.Events)
        {
            ...
            {
                _noteRaws.Add(new NoteRaw(ev.Timing, ev_hold.EndTiming - ev_hold.Timing));
            }
        ...
        {
            for (var i = arc_desc.Count - 1; i > 0; i--)
                if (arc_desc[i].Timing != arc_desc[i - 1].EndTiming)
                    _noteRaws.Add(new NoteRaw(arc_desc[i].Timing, 0));
            if (arc_desc.Any())
                _noteRaws.Add(new NoteRaw(arc_desc[0].Timing, 0));
        }
    ...
    public void AnalyzeSegments()
    {
        ...
        if (Timings.Count >= 1)
        {
            var segmentRemain = Timings[Timings.Count - 1].Bpm == 0
                ? totalTime - Timings[Timings.Count - 1].Timing
    ...
    public Dictionary<int, int> GetChartQuality(int threshold)
    {
        ...
        for (var i = 0; i < timing_list.Count; i++)
        {
            var (timing, ev) = timing_list[i];

            for (var t = 0; t < timing_list.Count; t++)
            {
