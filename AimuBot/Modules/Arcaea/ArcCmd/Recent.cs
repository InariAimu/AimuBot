...
namespace AimuBot.Modules.Arcaea
{
    ...
    {
        private static readonly Dictionary<int, string> _recentStyles = new()
        {
            { 0, "Arcaea" },
            { 1, "Phigros" },
        };
        ...
        public MessageChain OnSetRecentCardType(BotMessage msg)
        {
            ...
            if (t >= 0 && t < _recentStyles.Count)
            {
                _db.SaveInt(msg.SenderId, "recent_type", t);
                return $"recent 样式已设置为 v{t}：{_recentStyles[t]}";
            }
            else
            {
                return "请使用 /ac set <style_id> 设置 recent 卡片样式。\n" +
                    string.Join("\n", _recentStyles.Select((kvp) => $"v{kvp.Key}: {kvp.Value}"));
            }
        }
        ...
        public async Task<MessageChain> OnRecent(BotMessage msg)
        {
            ...
            var arcIdOrName = bindInfo.bind_type == 1 ? bindInfo.name : bindInfo.arc_id;
            ...
            UpdatePttHistory(bindInfo.arc_id, response.Content.AccountInfo.RealRating, response.Content.RecentScore[0].TimePlayed, 0);

            var im = GetRecentImage(response, bindInfo.arc_id, bindInfo.recent_type);
        ...
        internal Image? GetRecentImage_Phigros(Response recent, string arc_id)
        {
            ...
            string grade_text = GetGradeStr(play_info.Score, true);
            ...
            var high_scores = _db.GetObjects<ScoreDesc>(
                ...
                new() { { "$arc_id", arc_id }, { "$song_id", play_info.SongId }, { "$diff", play_info.Difficulty } }
                );
            ...
            if (high_scores.Length > 0)
            {
                ...
                int score_diff = play_info.Score - hs;
            ...
            ui.GetNodeByPath<LuiText>("right/good").Text = (play_info.PerfectCount - play_info.ShinyPerfectCount).ToString();
            ...
            float acc = (float)play_info.Score * 100 / 10000000;
            ...
            string? difft_str = song_info_raw_diff.Rating.ToString();
            if (song_info_raw_diff.RatingPlus)
                difft_str += "+";

            string rating_str = difft_str;
            if (rating_f > 0)
            {
                rating_str = GetRating(play_info.SongId, play_info.Difficulty).ToString("F1");
            }

            string? diff_str = GetShortDiffcultyStr(play_info.Difficulty);
            ...
            ui.GetNodeByPath<LuiText>("datetime").Text = DateTimeOffset.FromUnixTimeMilliseconds(play_info.TimePlayed).LocalDateTime.ToString();
        ...
        private Image? GetRecentImage_Arcaea(Response recent, string arc_id, int ranking = 0)
        {
            ...
            if (content.AccountInfo.Rating >= 0)
            {
                ui.GetNodeByPath<LuiText>("top_bar/user_info/ptt_bg/ptt_int").Text = (content.AccountInfo.Rating / 100).ToString() + ".";
                ui.GetNodeByPath<LuiText>("top_bar/user_info/ptt_bg/ptt_tail").Text = (content.AccountInfo.Rating % 100).ToString("D2");
            }
            else
            {
            ...
            ui.GetNodeByPath<LuiImage>("top_bar/user_info/ptt_bg").ImagePath = $"Arcaea/assets/img/rating_{rating_bg_no}.png";
            ...
            if (lastPttInfos != null && lastPttInfos.Length > 0)
            {
                ...
                if (ptt_diff_d > 0)
                {
                    ...
                    ui.GetNodeByPath<LuiText>("top_bar/user_info/pott_bg/ptt_add").BorderColor = Color.FromArgb(60, 128, 255, 255);
                    ui.GetNodeByPath<LuiImage>("top_bar/user_info/pott_bg").ImagePath = "Arcaea/assets/layouts/results/rating_up.png";
                }
                else if (ptt_diff_d == 0)
                {
                    ...
                    ui.GetNodeByPath<LuiImage>("top_bar/user_info/pott_bg").ImagePath = "Arcaea/assets/layouts/results/rating_keep.png";
                }
                else if (ptt_diff_d < 0)
                {
                    ui.GetNodeByPath<LuiText>("top_bar/user_info/pott_bg/ptt_add").Text = ptt_diff;
                    ui.GetNodeByPath<LuiText>("top_bar/user_info/pott_bg/ptt_add").BorderColor = Color.FromArgb(60, 255, 128, 128);
                    ui.GetNodeByPath<LuiImage>("top_bar/user_info/pott_bg").ImagePath = "Arcaea/assets/layouts/results/rating_down.png";
                }
            ...
            ui.GetNodeByPath<LuiText>("banner/score_board/score").Text = play_info.Score.ToString("D8").Insert(5, "\'").Insert(2, "\'");
            if (play_info.PerfectCount == play_info.ShinyPerfectCount && play_info.NearCount == 0 && play_info.MissCount == 0)
            {
            ...
            string? grade_text = GetGradeStr(play_info.Score);
            ...
            if (high_scores.Length > 0)
            {
                ...
                ui.GetNodeByPath<LuiText>("banner/score_board/hi_score").Text = hs.ToString("D8").Insert(5, "\'").Insert(2, "\'");

                int score_diff = play_info.Score - hs;
                string? score_diff_text = (score_diff >= 0 ? "+" : "") + score_diff.ToString("D8").Insert(5, "\'").Insert(2, "\'");
                ui.GetNodeByPath<LuiText>("banner/score_board/score_diff").Text = score_diff_text;

                if (score_diff <= 0)
                    ui.GetNodeByPath<LuiImage>("banner/score_board").ImagePath = $"Arcaea/assets/layouts/results/res_scoresection.png";
            }
            else
            {
            ...
            if (ranking > 0)
            {
                ui.GetNodeByPath<LuiText>("banner/score_board/hi_score").Text = (play_info.Score - 1).ToString("D8").Insert(5, "\'").Insert(2, "\'");
            ...
            {
                Color normalFontColor = Color.FromArgb(32, 32, 32);
                ...
                ui.GetNodeByPath<LuiText>("banner/perf/big_pure").Text = "+" + play_info.ShinyPerfectCount.ToString();
                ...
                ui.GetNodeByPath<LuiText>("banner/perf/far_detail").Text = $"(P{play_info.PerfectCount - play_info.ShinyPerfectCount})";
            ...
            int target_size_x = ui.Root.Option.CanvasSize.Width * 95 / 100;
            int target_size_y = ui.Root.Option.CanvasSize.Height * 95 / 100;
